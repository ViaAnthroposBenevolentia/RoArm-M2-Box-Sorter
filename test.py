...
if __name__ == '__main__':
    return_to_default()
    move_arm_to_position(ARM_PICK_POSITION)
    move_arm_to_position(ARM_CLOSE)
    move_arm_to_position(ARM_CAMERA_POSITION)
    color = detect_color_once()
    print(color)
    if color == 'Yellow':
        move_arm_to_position(ARM_YELLOW_BOX_POSITION)
        move_arm_to_position(ARM_YELLOW_BOX_RELEASE)
    elif color == 'Green':
        move_arm_to_position(ARM_GREEN_BOX_POSITION)
        move_arm_to_position(ARM_GREEN_BOX_RELEASE)
    else:
        move_arm_to_position(ARM_RED_BOX_POSITION)
        move_arm_to_position(ARM_RED_BOX_RELEASE)
    return_to_default()
    move_arm_to_position(ARM_CAMERA_POSITION)